...
def base_annotation(
    field: str, type: type | Annotated = str, optional: bool = False
) -> Annotated:
    if optional:
        return Annotated[
            type | None,
            Field(
                description=f"The {field.lower()} of the talent", title=field.title()
            ),
        ]
    return Annotated[
        type,
        Field(description=f"The {field.lower()} of the talent", title=field.title()),
    ]
...
programming_languages_annotation = base_annotation(
    "programming languages", type=list[str]
)
